...
* Memory-mapped lake map (`.br_map.bazan`): stores file metadata plus Parquet row-group and column-chunk locations, with automated drift detection (`br.lake.doctor`). It is not a per-row byte index.
