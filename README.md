...
* Memory-mapped lake map (`.br_map.bazan`): stores file metadata plus Parquet row-group/column-chunk locations, NDJSON/CSV/TSV block checkpoints, and Arrow IPC/Feather batch ordinals, with automated drift detection (`br.lake.doctor`). It is not a per-row byte index.
* Slicing (`br.read`): healthy Parquet maps resolve row groups, NDJSON/CSV/TSV maps seek to quote-safe row blocks, and Arrow IPC/Feather maps seek to RecordBatch ordinals before reading; other formats retain bounded streaming fallback without loading entire files into RAM.
