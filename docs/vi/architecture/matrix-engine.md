...
pub struct MatrixEngine {
    ...
    pub max_speed_mph: f64,   // mặc định 100.0; dùng khi có timestamp taxi
}
```
...
| Phương thức | Hành vi |
| :--- | :--- |
| `slice_rows(file_path, offset, limit)` | Parquet phân giải row group khỏe từ `.br_map.bazan`; NDJSON/JSONL và CSV/TSV/PSV/TXT nhảy tới block đã lập chỉ mục; Arrow IPC/Feather nhảy tới RecordBatch đã lập chỉ mục. Các nhánh này sau đó áp dụng offset/limit cục bộ; định dạng khác vẫn stream và bỏ qua batch. |
| `slice_cols(file_path, selected_cols, offset, limit)` | Như trên. Parquet đẩy phép chiếu xuống reader; NDJSON/JSONL và CSV/TSV/PSV/TXT cùng Arrow IPC/Feather đọc từ boundary đã lập chỉ mục rồi mới chiếu các cột yêu cầu. |
...
- **Đường tắt tĩnh** (`src/filter.rs` + `engine/filter.rs`): ba cờ bit cố định, khoảng passenger, mức giá tối thiểu và bất thường tốc độ/distance-fare. Passenger hỗ trợ cả dạng `Int64` và `Float64` có trong dữ liệu TLC. Khi có timestamp pickup/dropoff, tốc độ được tính từ khoảng cách và thời lượng; schema khác dùng fallback distance/fare cũ. Dùng cho `process_batch` / `process_file` / `process_and_write_lake` / `preview_sample`.
