...
| Lake Map | `src/engine/map.rs` | `LakeMap`, IO `.br_map.bazan`, `doctor_lake_map` |
...
| `.br_map.bazan` | Payload Lake map Arrow IPC tại gốc: metadata file cùng vị trí row-group Parquet, stripe ORC, block OCF Avro, block object MsgPack, block dòng logic XLSX, block NDJSON/JSONL/JSON-array và block dòng CSV/TSV/PSV/TXT và batch Arrow IPC/Feather (7 cột, xem đặc tả) |
