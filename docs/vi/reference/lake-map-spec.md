...
Lake Map được serialize thành **tệp Apache Arrow IPC** tên `.br_map.ipc` tại gốc hồ dữ liệu (`resolve_map_path()` trong `src/engine/map.rs`). Được `br.lake.create_map()` ghi ra và đọc qua memory-map (<1 ms). Map mới có vị trí row group Parquet; map cũ 5 cột vẫn đọc được nhưng không tăng tốc slicing. `doctor_lake_map` vẫn kiểm tra metadata tệp hiện tại để phát hiện drift.
