...
Lake Map được serialize thành **payload Apache Arrow IPC** trong tệp hệ thống `.br_map.bazan` tại gốc hồ dữ liệu (`resolve_map_path()` trong `src/engine/map.rs`). Được `br.lake.create_map()` ghi ra và đọc qua memory-map (<1 ms). Map mới có vị trí row group Parquet, checkpoint byte theo block dòng NDJSON/CSV/TSV/PSV và ordinal RecordBatch của Arrow IPC/Feather; map cũ 5 cột vẫn đọc được nhưng không tăng tốc slicing. `.br_map.ipc` được dành riêng làm tên sidecar cũ bị bỏ qua. `doctor_lake_map` vẫn kiểm tra metadata tệp hiện tại để phát hiện drift.
...
| `row_groups_json` | `Utf8` | Không | Mảng JSON vị trí row group Parquet, block dòng NDJSON/CSV/TSV/PSV hoặc RecordBatch Arrow IPC/Feather; `[]` với định dạng khác |

Mỗi object có `ordinal`, `first_row`, `row_count`, `first_byte`, `total_byte_size`, `compressed_size` và `columns`. Object Parquet có thêm path cột, byte range nén và page location tùy chọn. Object NDJSON/CSV/TSV/PSV dùng `first_byte` và `total_byte_size` cho block 64K dòng; checkpoint định dạng phân cách hiểu quote. Object Arrow IPC/Feather dùng ordinal và khoảng dòng, còn `first_byte`, `total_byte_size`, `columns` để rỗng vì reader dùng batch index native của Arrow IPC. Đây là vị trí block/row-group/batch, không phải byte offset của từng dòng.
