...
Lake Map được serialize thành **payload Apache Arrow IPC** trong tệp hệ thống `.br_map.bazan` tại gốc hồ dữ liệu (`resolve_map_path()` trong `src/engine/map.rs`). Được `br.lake.create_map()` ghi ra và đọc qua memory-map (<1 ms). Map mới có vị trí row group Parquet, checkpoint byte theo block dòng NDJSON/CSV/TSV và ordinal RecordBatch của Arrow IPC/Feather; map cũ 5 cột vẫn đọc được nhưng không tăng tốc slicing. `.br_map.ipc` được dành riêng làm tên sidecar cũ bị bỏ qua. `doctor_lake_map` vẫn kiểm tra metadata tệp hiện tại để phát hiện drift.
