...
| `.br_map.bazan` | Arrow IPC lake map payload at root: file metadata plus Parquet row-group, ORC stripe, Avro OCF block, MsgPack object block, XLSX logical row block, NDJSON/JSONL/JSON-array and CSV/TSV/PSV/TXT block, and Arrow IPC/Feather batch locations (7 columns, see spec) |
