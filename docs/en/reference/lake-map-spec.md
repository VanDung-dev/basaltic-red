...
| `row_groups_json` | `Utf8` | No | JSON array of Parquet row groups, NDJSON/JSONL and CSV/TSV/PSV/TXT row blocks, or Arrow IPC/Feather RecordBatches; `[]` for other formats |

Each object contains `ordinal`, `first_row`, `row_count`, `first_byte`, `total_byte_size`, `compressed_size`, and `columns`. Parquet objects additionally contain column paths, compressed byte ranges, and optional page locations. NDJSON, JSONL, CSV, TSV, PSV, and TXT objects use `first_byte` and `total_byte_size` for 64K-row blocks; delimited checkpoints are quote-safe. Arrow IPC/Feather objects use the ordinal and row range, with `first_byte`, `total_byte_size`, and `columns` empty because the reader uses Arrow IPC's native batch index. These are block/row-group/batch locations, not individual row byte offsets.
